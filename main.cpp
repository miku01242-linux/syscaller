...
using NtFreeVirtualMemory_t = NTSTATUS(WINAPI*)(HANDLE, PVOID*, PSIZE_T, ULONG);
...
int main() {
    ...
    printf("[syscall] status -> 0x%ld\n", status);
    printf("[syscall] allocation -> %p\n", alloc);
    ...
    auto NtFreeVirtualMemory = syscaller::syscall<NtFreeVirtualMemory_t>("NtFreeVirtualMemory");
    NTSTATUS freeStatus = NtFreeVirtualMemory(
        GetCurrentProcess(),
        &alloc,
        &size,
        MEM_RELEASE
    ...
    printf("[syscall] free status -> 0x%ld\n", freeStatus);
